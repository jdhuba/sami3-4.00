...
!    w(2) = zonal (m/sec + eastward)
    ...
    ! Arguments
    real,    intent(in) :: hr
    integer, intent(in) :: nll, iday

    ! Local variables
    integer :: i, j, k, n
    integer :: iyd          ! MSIS year-day parameter (output of msistim)
    real    :: sec          ! MSIS universal time in seconds (output of msistim)
    real    :: glonsij, hrl ! geographic longitude and local time for each grid point
    ...
    do j = 1,nf
        do i = 1,nz
            ...
            call hwm14 ( iyd,sec,alts(i,j,nll), &
                                     glats(i,j,nll),glonsij, &
...
!   secut  the universal time in seconds
    ...
    real,    intent(in)  :: hr, glong
    integer, intent(out) :: iyd
    real,    intent(out) :: secut

    ! Local variable
    real :: hrut   ! working UT hours (local scope; distinct from time_mod hruti/hrutf)
