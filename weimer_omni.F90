...
! this must be linked to Geopack.o (Geopack.f from NASA Goddard)
! This version takes quantities from the ACE OMNI dataset (level 2 combined 1-min data)
      ...
      ! Arguments
      integer, intent(in) :: imonth_1, iday_1, imonth_2, iday_2, iyear0

      ! fix #5: nlines defined as nwind0 -- eliminates duplicate magic constant
      integer, parameter :: nwind0 = 525601
      integer, parameter :: nlines = nwind0

      ! fix #7: name the 16th GEOPACK1 element (PSI = dipole tilt angle in radians)
      !         so access via PSI_RAD is documented and robust; AA covers elements 1-15

      real :: AA,PSI_RAD

      COMMON /GEOPACK1/ AA(15), PSI_RAD

      ! fix #10: full-precision pi computed once
      ...
      ! fix #1: all previously implicit locals declared explicitly
      integer :: i, ii, j, isb, isp
      integer :: i_start, i_end         ! fix #3: initialised below
      integer :: iday, iday0, iday_g
      integer :: ihour, imin, isec      ! fix #2: isec is integer
      integer :: navg, npts, nout
      integer :: ilg, ifg, iyr_i
      real    :: earthr, xcraft, npskip, cadence, window
      real    :: time, time1, time2, twin1, twin2
      real    :: bysum, bzsum, bylg, bzlg, byfg, bzfg, fac
      real    :: dsum, denlg, denfg
      real    :: vxsum, vxlg, vxfg
      real    :: vbigsum, vavg, tshift
      real    :: vgsex, vgsey, vgsez, angrot
...
!     Open IMF file and skip header lines
      ...
      ! fix #3: initialise sentinel values so missing dates are detectable
      i_start = 0
      i_end   = 0
      ...
      ! fix #3: abort with a clear message if start or end date was not found
      if (i_start == 0) then
        print *, 'ERROR: start date not found in OMNI file -- check imonth_1,iday_1,iyear0'
        ERROR STOP 1
      endif
      if (i_end == 0) then
        print *, 'ERROR: end date not found in OMNI file -- check imonth_2,iday_2,iyear0'
        ERROR STOP 1
      endif
...
!     Re-open IMF file and skip to start record
      open(33, FILE=pwind, STATUS='OLD', FORM='FORMATTED')
      do i = 1, npskip+i_start-1
        read(33,*)
      enddo

!     Read the actual data
!     (bygse/bzgse contain OMNI GSM By,Bz despite the GSE variable names)
      iday = 0
      ...
      do ii = i_start, i_end
        ...
        ! Fill in timeline (decimal minutes)
    ...
    3 format(i2,9x,i2,1x,i2,1x,f6.3,i12,i15,2f14.5,f17.3,f15.5,f14.3&
      ...
      write(*,*) ' h:m:s ',ihr(npts),minu(npts),sec(npts)
      write(*,*) ' By Bz ',bygse(npts),bzgse(npts)
      write(*,*) ' Vx n  ',vxgse(npts),denin(npts)
      write(*,*) ' ID of END ',id(npts)
...
!     Loop over times for By, Bz
      do j = 1, nout
        ...
        do
          i = i + 1
          ! fix #4: guard against overrunning the data array
          if (i > npts) exit
        ...
        if (navg .gt. 3) then
          ! fix #6: float(navg) -> real(navg)
          by(j) = bysum / real(navg)
          bz(j) = bzsum / real(navg)
        else
          ! Sparse data: interpolate between last good and next good value
          i = ilg
          byfg = bylg
          bzfg = bzlg
          ifg  = ilg
          do
            i = i + 1
            ! fix #4: guard against overrunning
            if (i > npts) exit
          ...
          enddo
          fac   = (time - timein(ilg)) / max(timein(ifg) - timein(ilg), 1.0e-10)
          by(j) = bylg - (bylg - byfg)*fac
          bz(j) = bzlg - (bzlg - bzfg)*fac
        endif
      enddo

!     Loop over times for density
      do j = 1, nout
...
!     Shift times based on average solar wind speed
      tshift = (xcraft - 0.0)*earthr / (vavg*3600.0)
      hrutd  = hrutd + tshift

      print *, ' Vavg (km/s), time shift (h) = ', vavg, tshift

!     Dipole tilt angle (uses Geopack RECALC_08 -> COMMON /GEOPACK1/ element 16 = PSI)
      do j = 1, nout
        iday  = int(hrutd(j)/24.0)
        ihour = int(hrutd(j) - iday*24.0)
        imin  = int(hrutd(j)*60.0 - ihour*60.0 - iday*24.0*60.0)
        isec  = 0           ! fix #2: integer 0, not real 0.0
        iday_g = iday + iday0

!       RECALC_08 sets dipole tilt; vgsex < 0 gives standard GSM (anti-sunward)
        vgsex = -400.
        vgsey = 0.
        vgsez = 0.
        iyr_i = iyear0
        ...
        ! fix #7: PSI_RAD is the named alias for GEOPACK1 element 16 (dipole tilt)
        ! fix #10: 3.14159 replaced with full-precision pi = acos(-1.0)
        tilt(j) = PSI_RAD * 180.0 / pi

        if (j .eq. 1) then
          print *, 'stuff', iyr_i, iday_g, ihour, imin, isec, vgsex, vgsey, vgsez
        endif
      enddo
...
!     Write output
   ...
   43 format(6f15.5)
      ...
      end subroutine weimer_omni   ! fix #13
