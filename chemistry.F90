...
!     bb: bailley and balan (red book, 1996)
    ...
    ! Local variables
    integer :: iz
    real    :: ti300   ! ion temperature normalized to 300 K

    do iz = 1, nz

        ! Guard against non-positive temperature
        ti300 = max( ti(iz,nfl,nll,ptop), 1.0 ) / 300.0
    ...
    ! n2+ + o --> no+ + n(2d) (bb)
        ...
        chrate(iz,14) = 1.4e-10 / ti300 ** 0.44

    ! n2+ + o2 --> o2+ + n2 (schunk)

        chrate(iz,15) = 5.0e-11 / sqrt( ti300 )
...
! bb: bailley and balan (red book, 1996)
    ...
    ! Arguments
    integer, intent(in)  :: nfl, nll
    real,    intent(out) :: relossr(nz,nion)

    ! Local variables
    integer :: iz
    real    :: te300   ! electron temperature normalized to 300 K

    do iz = 1, nz

        ! Guard against non-positive temperature
        te300 = max( te(iz,nfl,nll), 1.0 ) / 300.0
...
!        species for each reaction
    ...
    ! Arguments
    integer, intent(in)  :: nfl, nll
    real,    intent(in)  :: chrate(nz,nchem)
    real,    intent(out) :: chloss(nz,nion)
    real,    intent(out) :: chprod(nz,nion)

    ! Local variables
    integer :: iz, i, k, il, in, ip
    real    :: chem, tdeni

    ! Initialize production and loss arrays
    chloss(:, nion1:nion2) = 0.0
    chprod(:, nion1:nion2) = 0.0
    ...
    do k = 1, nchem
        ...
        do iz = 1, nz
            ...
            chloss(iz,il) = chloss(iz,il) + tdeni
            chprod(iz,ip) = chprod(iz,ip) + tdeni
        enddo
    enddo
