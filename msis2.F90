...
subroutine bsplinex(x,nodes,nd,kmax,eta,S,i)
  ...
  ! FIX: nodes was hardcoded (0:29) — incompatible with nodesO1/nodesNO (0:13).
  !      Changed to assumed-size (0:*) so all callers (nodesTN, nodesO1, nodesNO)
  !      are accepted without shape mismatch errors under modern gfortran.
  !      The actual upper bound is always passed and used via the 'nd' argument.
  real(kind=rp), intent(in)  :: nodes(0:*)      !Spline node locations (assumed-size)
  ...
  ! FIX: eta was hardcoded (0:30,2:6). Changed to assumed-size in first dim to
  !      match etaTN/etaO1/etaNO all declared (0:30,2:6) — kept explicit for
  !      second dim so indexing by order (2:6) remains valid.
  real(kind=rp), intent(in)  :: eta(0:30,2:6)  !Precomputed reciprocal node-difference weights
  ! Ouput variables
  real(kind=rp), intent(out) :: S(-5:0,2:6)    !B-spline values (index rel. to i, order 2:6)
  integer, intent(out)       :: i              !Index of last nonzero b-spline

  ! Working variables
  ! FIX: removed unused 'k' (only appeared in commented-out generic loop).
  !      removed unused 'sb(-5:0)' scratch array (declared but never referenced).
  integer                    :: j, l
  integer                    :: low, high
...
! =============================
! Manually unrolled recursion
! (replaces the generic loop)
! =============================
